...
Player.prototype.qipai = function(data) {
  ...
  this._shoupai = Shoupai.fromString(data.shoupai[this._menfeng])
  this._menqian = true
  this._dapai = {}
  ...
  this._suanpai = new Suanpai(this._chang.hongpai)
...
Player.prototype.dapai = function(data, callback) {
  ...
  if (data.l == this._menfeng) {
    ...
    if (utilXiangting(this._shoupai) == 0) {
      for (var p of utilTingpai(this._shoupai)) {
  ...
  if (utilXiangting(new_shoupai) == -1) {
...
Player.prototype.get_gang_mianzi = function(data) {
  ...
  if (data) {
    ...
    if (this._lizhi[this._menfeng] && mianzi.length) {
      ...
      var tingpai = utilTingpai(new_shoupai).join(',')
      ...
      for (var m of mianzi) {
        ...
        new_shoupai.gang(m)
        if (
          utilXiangting(new_shoupai) == 0 &&
          utilTingpai(new_shoupai).join(',') == tingpai
        ) {
          return [m]
        }
      }
...
Player.prototype.allow_lizhi = function(p) {
  if (
    ...
    if (p == null) return true
      ...
      return (
        utilXiangting(new_shoupai) == 0 &&
        utilTingpai(new_shoupai).length > 0
      )
    }
  }
  return false
...
Player.prototype.allow_hule = function(data, option) {
  ...
  if (utilXiangting(new_shoupai) != -1) return false
  ...
  var hule = utilHule(new_shoupai, rongpai, param)
...
Player.prototype.select_fulou = function(data) {
  ...
  var n_xiangting = utilXiangting(this._shoupai)
  ...
  if (n_xiangting < 3) {
    ...
    for (var m of mianzi) {
      var new_shoupai = this._shoupai.clone()
      new_shoupai.fulou(m)
      if (utilXiangting(new_shoupai) >= 3) continue
...
Player.prototype.select_pingju = function() {
  if (utilXiangting(this._shoupai) < 4) return false
...
Player.prototype.select_dapai = function() {
  ...
  for (var p of this.get_dapai()) {
    ...
    var new_shoupai = this._shoupai.clone()
    new_shoupai.dapai(p)
    if (utilXiangting(new_shoupai) > n_xiangting) {
    ...
    var n_tingpai = 0
    for (var tp of utilTingpai(new_shoupai)) {
      n_tingpai += paishu[tp]
    }
...
Player.prototype.xiangting = function(shoupai) {
  ...
  function xiangting_duanyao(shoupai) {
    ...
    return utilXiangting(new_shoupai)
  }
...
Player.prototype.tingpai = function(shoupai) {
  ...
  var pai = []
  for (var p of utilTingpai(shoupai, function(s) {
...
Player.prototype.get_defen = function(shoupai) {
  ...
  var hule = utilHule(shoupai, null, param)
...
Player.prototype.eval_shoupai = function(shoupai, paishu, dapai) {
  ...
  var rv
  var n_xiangting = utilXiangting(shoupai)
  ...
  if (n_xiangting == -1) {
    ...
    for (var p of get_dapai(shoupai)) {
      var new_shoupai = shoupai.clone()
      new_shoupai.dapai(p)
      if (utilXiangting(new_shoupai) > n_xiangting) continue
    ...
    var r = 0
    for (var p of add_hongpai(utilTingpai(shoupai))) {
      if (p == dapai) return 0
