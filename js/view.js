...
let View = {
  imgHtml: function(p) {
    return p
      ? '<img class="pai" data-pai="' + p + '" src="img/' + p + '.gif">'
 ...
 *  View.Shoupai
 */
var imgHtml = View.imgHtml

View.Shoupai = function(node, shoupai, open) {
  this._node = node
  this._shoupai = shoupai
  this._open = open
}

View.Shoupai.prototype.redraw = function() {
  ...
  for (var m of this._shoupai._fulou) {
    ...
    if (m.replace(/0/g, '5').match(/^[mpsz](\d)\1\1\1$/)) {
      ...
      var d = m.match(/[\-\+\=]/).shift()
      var img = m.match(/\d/g).map(function(n) {
        return imgHtml(s + n)
      })
      var img_r =
        '<span class="rotate">' +
      ...
      if (d == '-') html += img_r + img[0] + img[1] + (daminggang ? img[2] : '')
      if (d == '=') html += img[0] + img_r + img[1] + (daminggang ? img[2] : '')
      if (d == '+') html += img[0] + img[1] + (daminggang ? img[2] : '') + img_r
    } else {
...
View.Shoupai.prototype.dapai = function(p) {
  var bingpai = this._node.find('.bingpai')
...
View.Shoupai.prototype.open = function() {
  this._open = true
  this.redraw()
}
...
View.Shan = function(node, shan) {
  this._node = node
  this._shan = shan
}

View.Shan.prototype.redraw = function() {
  var baopai = this._shan.baopai()
  this._node.find('.baopai .pai').each(function(i) {
    $(this).after(i < baopai.length ? $(imgHtml(baopai[i])) : $(imgHtml()))
    $(this).remove()
  })

  var fubaopai = this._shan.fubaopai()
  this._node.find('.fubaopai .pai').each(function(i) {
    $(this).after(i < fubaopai.length ? $(imgHtml(fubaopai[i])) : $(imgHtml()))
    $(this).remove()
  })
...
View.He.prototype.redraw = function() {
...
View.He.prototype.dapai = function(p) {
  var dapai = this._node.find('div.dapai')
  var img = imgHtml(p.substr(0, 2))
...
View.Chang.prototype.redraw = function() {
  ...
  this._node
    .find('.title')
    .text(
      feng_hanzi[this._chang.zhuangfeng] + shu_hanzi[this._chang.jushu] + '局'
    )
...
View.Chang.prototype.update = function(l) {
  var view_class = ['main', 'xiajia', 'duimian', 'shangjia']
  var id = (this._chang.qijia + this._chang.jushu + l) % 4
  var c = view_class[(id + 4 - this._viewpoint) % 4]
  this._node.find('.defen .lunban').removeClass('lunban')
...
View.Jiesuan = function(node, shan, chang, viewpoint) {
  this._node = node
  this._shan = shan
  this._chang = chang
  this._viewpoint = viewpoint
...
View.Jiesuan.prototype.fenpei = function(info) {
  var feng_hanzi = ['東', '南', '西', '北']
  var view_class = ['main', 'xiajia', 'duimian', 'shangjia']
...
View.Jiesuan.prototype.hule = function(info) {
  this.hide()

  if (this._shan) {
    new View.Shan(this._node.find('.shan'), this._shan).redraw()
  ...
  new View.Shoupai(shoupai, info.shoupai, true).redraw()
  shoupai.show()
  ...
  if (!info.hule.hupai) {
    ...
    for (var h of info.hule.hupai) {
      var node = $(
        '<tr><td class="name"></td>' + '<td class="fanshu"></td></tr>'
      )
      node.find('.name').text(h.name)
      if (info.hule.damanguan) node.find('.fanshu').text(h.fanshu)
    ...
    if (info.hule.fanshu >= 13) text += '数え役満 '
      ...
      (info.lunban == 0 && info.hule.defen >= 12000) ||
...
View.Jiesuan.prototype.pingju = function(info) {
  this.hide()
...
View.Jiesuan.prototype.hide = function() {
  this._node.hide()
...
View.Jiezhang = function(node, paipu) {
  var feng_hanzi = ['東', '南', '西', '北']
  var shu_hanzi = ['一', '二', '三', '四']
  ...
  for (var ju of paipu.log) {
    ...
    var jieguo = ju.filter(function(log) {
      return log.pingju || log.hule
    })
  ...
  tr = $('<tr></tr>')
    .append($('<td colspan="3" class="point"></td>'))
    .appendTo(tfoot)
  for (var id = 0; id < 4; id++) {
...
View.Controler.prototype.update = function() {
  var self = this

  this._node.find('.sound .scale span').each(function(i) {
    if (i >= AudioObj.volume()) $(this).addClass('off')
